...
## 6. 자료창고 등록용 텍스트 제공 (수동 등록)

**자료창고(https://jaryo-changgo.vercel.app/)는 이 저장소(pengmomt)와는 완전히 별개의 독립된
앱이다.** Git 연동이 없고 Vercel CLI로 직접 배포되는 방식이라, 이 세션에서는 소스코드를 볼 수도
없고 그 도메인에 네트워크 접근도 막혀 있어 API로 자동 등록하는 것이 불가능하다. **절대 자료창고에
직접 배포하거나 그 프로젝트를 건드리려고 시도하지 않는다** — 대신 사용자가 자료창고의
"+ 새 자료 등록" 버튼으로 바로 붙여넣을 수 있는 텍스트를 만들어 채팅으로 제공한다.

배포 안내 메시지 뒤에 아래 형식으로 덧붙인다:

```
📋 자료창고에도 등록하시려면 아래 내용을 "+ 새 자료 등록"에 복사해 넣어주세요:

카테고리: 웹앱
제목: <title>
설명: <problem을 한 문장으로 요약 + features 핵심 1~2개>
태그: #<tag1> #<tag2> #<핵심기능 키워드>
링크: <previewUrl이 있으면 배포된 실제 URL, 예: https://pengbox-kifyss-alts-projects.vercel.app/games/<slug>/index.html>
```

- 카테고리는 결과물 성격에 맞게 `웹앱`/`스킬`/`프롬프트`/`연수자료` 중 하나로 판단해서 채운다
  (새로 만든 게임·도구는 보통 `웹앱`)
- 설명은 자료창고 카드에 그대로 노출되므로 과장 없이 간결하게 작성한다
